...
class EphysToNWBConverter:
    ...
    def convert_all_shanks(self, data_files: list, shanks: list, session_description: str,
                           data_folder: Path, impedance_path: Path = None,
                           bad_ch_ids: list = None, metadata: dict = None) -> dict:
        """
        Convert all shanks simultaneously. For each chunk, loads data for ALL shank
        channels in a single get_traces() call, then slices and writes to each shank's NWB.

        Returns: dict mapping shank index -> good_channel_ids
        """
        metadata = metadata or {}
        device_type = metadata.get("device_type", "4shank16intan" if self.recording_method == 'intan' else "4shank16")
        electrode_location = metadata.get("electrode_location", None)
        ...
        # --- Step 1: Prepare per-shank electrode config ---
        # Read impedance file once if provided
        ...
        shank_setups = {}
        for ish in shanks:
            channel_index, xcoord, ycoord = self.get_ch_index_on_shank(ish, device_type)
            ...
            if impedance_table is not None or self.recording_method == 'intan':
                good_channel_ids = electrode_df['channel_name'].tolist()
            else:
                good_channel_ids = electrode_df['channel_index'].tolist()
        ...
        # --- Step 2: Build combined channel list (union, preserving order) ---
        all_channel_ids = []
        seen = set()
        for ish in shanks:
        ...
        # Map each shank's channels to column indices in the combined array
        ...
        # --- Step 3: Get recording info once ---
        print("\nGetting recording information...")
        recording, sampling_freq, num_frames, conversion, offset = self._get_recording_info(
            first_file, selected_geom)

        chunk_frames = int(self.chunk_duration * sampling_freq)
        num_chunks = int(np.ceil(num_frames / chunk_frames))
        estimated_gb = self._estimate_file_size_gb(num_frames, len(all_channel_ids))

        print(f"Total channels across all shanks: {len(all_channel_ids)}")
        print(f"Duration: {num_frames / sampling_freq:.2f}s  |  Estimated size: {estimated_gb:.2f} GB")
        print(f"Processing {num_chunks} chunk(s) of {self.chunk_duration}s each")

        # --- Step 4: Create NWB shells using first chunk ---
        session_start_time = self.get_timestamp(first_file)

        def _write_initial_nwb(chunk_all, file_index, total_files):
            print(f"\nReading chunk 1/{num_chunks} for all shanks (file {file_index}/{total_files})...")
            for ish in shanks:
                setup = shank_setups[ish]
                electrode_df = setup['electrode_df']
                ...
                electrode_group = nwbfile.create_electrode_group(
                    name=f"shank{ish}",
                    description=f"electrode group for shank {ish}",
                    device=device,
                    location=electrode_location,
                )
                for _, row in electrode_df.iterrows():
                    nwbfile.add_electrode(
                        group=electrode_group,
                        label=f"shank{ish}:{row['channel_name']}",
                        location=electrode_location,
                        rel_x=float(row['x']),
                        rel_y=float(row['y']),
                        imp=float(row['impedance']) if not np.isnan(row['impedance']) else 0.0,
                ...
                electrical_series = ElectricalSeries(
                    name="ElectricalSeries",
                    data=H5DataIO(data=chunk_sh, maxshape=(None, chunk_sh.shape[1]),
                                  compression='gzip', compression_opts=4, chunks=True),
                    electrodes=electrode_table_region,
                    starting_time=0.0,
                    rate=sampling_freq,
        ...
        def _append_chunk_to_all_shanks(chunk_all):
            for ish in shanks:
                setup = shank_setups[ish]
                chunk_sh = chunk_all[:, setup['col_indices']]
                with NWBHDF5IO(setup['nwb_path'], "a") as io:
                    nwb_obj = io.read()
                    self._append_nwb_dset(
                        nwb_obj.acquisition['ElectricalSeries'].data, chunk_sh, 0)
                    io.write(nwb_obj)

        def _process_recording(rec, n_frames, file_label):
            ...
            for i in range(n_chunks):
                ...
                chunk_all = rec.get_traces(
                    channel_ids=all_channel_ids, start_frame=start, end_frame=end)
                yield chunk_all
                del chunk_all

        # First file: write NWB shells with first chunk, then append the rest
        gen = _process_recording(recording, num_frames, first_file.name)
        first_chunk = next(gen)
        _write_initial_nwb(first_chunk, file_index=1, total_files=len(data_files))
        del first_chunk

        for chunk_all in gen:
            _append_chunk_to_all_shanks(chunk_all)

        print(f"\nFile 1/{len(data_files)} ({first_file.name}) done.")

        # --- Step 5: Append additional data files ---
        for file_idx, f in enumerate(data_files[1:], 2):
            print(f"\n{'='*60}")
            print(f"Processing file {file_idx}/{len(data_files)}: {f.name}")
            print(f"{'='*60}")
            t0 = time.time()

            rec2, _, num_frames2, _, _ = self._get_recording_info(f, selected_geom)
            for chunk_all in _process_recording(rec2, num_frames2, f.name):
                _append_chunk_to_all_shanks(chunk_all)
...
def main():
    ...
    # Process all shanks simultaneously — one get_traces() call per chunk covers all shanks
    t0 = time.time()
